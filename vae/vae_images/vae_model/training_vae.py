...
def loss_function_individual(recon_x, x, mu, logvar,  beta):
    ...
    BCE = F.binary_cross_entropy(recon_x, x, reduction='none').mean(axis=(1, 2, 3))
    KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), dim=1)
    individual_loss = BCE + KLD*beta
    return individual_loss, BCE.detach().mean(), KLD.detach().mean()

def train_vae(train_loader, vae, predictors, optimizer_vae, delta, beta,rho,lambda_norm,  device):
    vae.train()
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        predictors_loss = F.binary_cross_entropy(decoded_estimated, data, reduction='none').mean(axis=(1, 2, 3)) #todo check if this is correct
