...
def generate_estimated_z(predictors, z):
    z_copy=z.clone().cpu()
    z_estimated = [z_copy[:, 0].unsqueeze(1)]  # Start with z_0_original
...
def autoencoder_step(train_loader, vae, predictors, optimizer_vae, delta,annealing_agent,beta_max,device):
    autoencoder.train()
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        predictors_loss_individual_examples = F.mse_loss(decoded_estimated, decoded, reduction='none').mean(axis=1)
...
def train_models(train_loader, val_loader, vae_model, predictors, criterion,
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            loss_predictors = F.mse_loss(decoded_estimated, decoded)
...
if __name__ == "__main__":
    ...
    torch.autograd.set_detect_anomaly(True)
    train_loader = get_dataloader(r"C:\Users\User\Desktop\thesis\data\toy_dataset_test.csv",
                                  batch_size=16384)
    ...
    optimizer_ae = optim.Adam(vae_model.parameters(), lr=0.000001)
    learning_rate_predictors =0.00001
    ...
    train_models(train_loader, val_loader, vae_model,
                 ...
                 scheduler_ae, schedulers, num_epochs=10000,
                 log_dir=log_dir, delta=0.001, beta_vae=0.00001,beta_max = 0,
